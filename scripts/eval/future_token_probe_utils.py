#!/usr/bin/env python3
"""Shared utilities for future-token linear probe experiments."""
...
def run_probe_experiment(
    args: argparse.Namespace,
    dataset_label: str,
    build_examples_fn: Callable[[object, argparse.Namespace], Tuple[List[Dict[str, object]], List[Dict[str, object]]]],
) -> None:
    set_seed(args.seed)
    ...
    print(f"Preparing {dataset_label} examples...")
    train_examples, eval_examples = build_examples_fn(tokenizer, args)
    ...
    print(
        f"Inferred l' (final encoder-below-decoder layer) = {lprime_layer} "
    ...
    print(f"\\n=== Future-Token Probe Summary ({dataset_label}, Eval) ===")
    for r in rows:
        print(
            f"t+{r['horizon']}: "
