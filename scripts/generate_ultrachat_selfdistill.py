#!/usr/bin/env python3
"""Self-distill UltraChat user prompts with optional batching and GPU sharding.
...
def main() -> int:
    ...
    if args.num_shards == 1 and (root.with_name(root.name + ".shards") / "run_config.json").exists():
    ...
    if metadata_path.exists():
        ...
        if (previous.get("num_shards", 1) != args.num_shards
                ...
                or previous.get("generation_num_shards", args.num_shards) != args.num_shards):
    ...
    if args.batch_size > 1:
        metadata.update(padding_side="left", generation_num_shards=args.num_shards)
    ...
    print(f"[distill] UltraChat: {source_rows} source rows across {args.source_splits}; "
          f"resumed {len(rows)}; batch_size={args.batch_size}; completion_budget={args.max_length // 2}; "
    ...
    with raw_path.open("a", encoding="utf-8") as output:
        ...
        def flush_batch():
            ...
            rows.extend(batch)
            pending.clear()
            if len(rows) // args.progress_every > previous_count // args.progress_every:
                print(f"[distill] {len(rows)} generated; source {rows[-1]['source_index'] + 1}/{source_rows}",
                      flush=True)
        ...
        for split, source in sources.items():
            ...
            for source_row_index in range(first, len(source), args.num_shards):
                ...
                if source_index <= resume_through:
                    continue
                if args.max_samples and len(rows) + len(pending) >= args.max_samples:
                    break
                example = source[source_row_index]
                prompt = build_prompt(example, tokenizer)
                if prompt is None:
                    skipped += 1
                    continue
                ...
                if len(prompt_ids) > metadata["max_prompt_tokens"]:
                    skipped += 1
                    continue
                pending.append({
                    "source_index": source_index, "source_split": split,
                    "source_row_index": source_row_index,
