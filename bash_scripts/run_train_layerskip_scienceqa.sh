...
# Model arch
N_LAYERS=28  # Use all 28 layers of Qwen3-1.7B-Base
...
RUN_NAME=scienceqa-${NUM_SHOT}shot_lr${LR}_bsz${BATCH_SIZE}_warm${WARMUP_DURATION}_alphaf${ALPHA_F}_max-dur${MAX_DURATION}_${PRECISION}_${LAYERS}_${TAG}_${TIMESTAMP}
...
uv run composer -n ${NUM_VISIBLE_DEVICES} scripts/composer_scripts/train_discrete_denoiser.py \
  ...
  dataset@train_dataset=scienceqa_train \
  dataset@eval_dataset=scienceqa_eval \
